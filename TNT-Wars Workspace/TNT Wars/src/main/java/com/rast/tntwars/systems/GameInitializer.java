...
public class GameInitializer {
	...
	private boolean checkStartGameConditions () {
		...
		return teamOneCount >= minTeamSize && teamTwoCount >= minTeamSize;
	}
	...
	private void startGame() {
		...
		taskID = Bukkit.getScheduler().scheduleSyncRepeatingTask(tntwars, () -> {
			if (timeInSeconds > 0) {
				...
				for (Map.Entry<String,String> entry : playerMap.entrySet()) {
					...
					if (Objects.equals(group, "teamOne")) {
						PlayerTeleportManager.teleportTeamOneIsland(Objects.requireNonNull(player), false);
						player.getInventory().addItem(TNTWarsMain.itemStackStorage.blockMenuItem);
					} else if (Objects.equals(group, "teamTwo")) {
						PlayerTeleportManager.teleportTeamTwoIsland(Objects.requireNonNull(player), false);
						player.getInventory().addItem(TNTWarsMain.itemStackStorage.blockMenuItem);
					}
				}
