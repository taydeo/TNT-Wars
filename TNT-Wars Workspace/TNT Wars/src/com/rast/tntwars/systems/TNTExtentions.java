...
public class TNTExtentions {
	...
	private static World mainWorld = TNTWarsMain.configEngine.gameWorld;
	private static BukkitScheduler scheduler = Bukkit.getScheduler();
	private static ConfigEngine configEngine = TNTWarsMain.configEngine;
	private static boolean tntTeamGriefProtection = configEngine.tntTeamGriefProtection;
	private static boolean destructableWater = configEngine.destructableWater;
	private static Material water = Material.WATER;
	private static Material air = Material.AIR;
	private static Material tnt = Material.TNT;
	private static EntityType primedTNT = EntityType.PRIMED_TNT;
	...
	public static void tntExplodeEventHandle (EntityExplodeEvent event) {
		...
		if (tntTeamGriefProtection && entity.getMetadata("regOfPrimed").get(0).asString() == RegionManager.regionDetect(location)) {
			...
			if (destructableWater) {
				...
				scheduler.runTask(tntwars, new Runnable() {
					@Override
					public void run() {
						if (mainWorld != null) {
